...
public class MailTest {
	...
	public void sendEmail(String from, String to, String cc, String subject,
			String content,int[] code) throws Exception {
		...
		/**
		 * SMTP 인증이 필요한 경우 반드시 Properties 에 SMTP 인증을 사용한다고 설정하여야 한다. 그렇지 않으면 인증을
		 * 시도조차 하지 않는다. 그리고 Authenticator 클래스를 상속받은 SMTPAuthenticator 클래스를 생성한다.
		 * getPasswordAuthentication() 메소드만 override 하면 된다. 머 사실 다른 메소드는 final
		 * 메소드여서 override 할 수 조차 없다. -ㅅ-;
		 */
		...
		msg.setFrom(new InternetAddress("psmco9@gmail.com"));// 보내는 사람 설정
		msg.setRecipients(Message.RecipientType.TO,
				InternetAddress.parse(to, false));// 받는 사람설정
		...
		msg.setSubject("코스타 테스트"); // 제목 설정
		msg.setText("인증코드 : "+Arrays.toString(code)); // 내용 설정
