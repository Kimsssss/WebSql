       ...
       $( "#sections" ).droppable({
         ...
         accept: "#createtable",
          drop: function( event, ui ) {
           ...
           //다이얼 로그 생성
           $(this).append("<div hidden id='constraintDialog"+ tableNumber +"_"+ colNumber +"' title='제약조건'>PRIMARY KEY : <input type='radio' name='constraint_ck' id='constraint_ck' value='PRIMARY KEY'>&nbsp;&nbsp;  FOREIGN KEY :  <input type='radio' name='constraint_ck' id='constraint_ck' value='FOREIGN KEY'>&nbsp;&nbsp;   NOT NULL : <input type='radio' name='constraint_ck' id='constraint_ck' value='NOT NULL'> </div>");
          ...
          $(document).on('click',"#constraint"+ tableNumber +"_"+ colNumber ,function() {
           ...
           /* 클릭시 컬럼 추가 */
             $(document).on('click',"#plus"+tableNumber ,function() {
               ...
               //컬럼을 추가 한다
               $(this).parent().parent().parent().parent().append("<tr><td><img id='colimg' name='colimg' src='<%=request.getContextPath()%>/resources/img/2.jpg'><input type='text' name='col"+ tableNumber +"_"+ ++colNumber +"'></td><td><input type='text' name='col_data"+ tableNumber +"_"+ colNumber +"'></td><td id='cons'><input type='button' class='btn btn-outline btn-primary btn-xs' id='constraint"+ tableNumber +"_"+ colNumber +"' name='constraint' value='제약설정'><input type='hidden' id='conhidden' value='constraintDialog"+ tableNumber +"_"+ colNumber +"'><input type='hidden' id='conhidden_hidden' value='constraintDialog_Dialog"+ tableNumber +"_"+ colNumber +"'><input type='hidden' id='constraints' value=''><input type='hidden' id='hidden_tableName' value=''><input type='hidden' id='hidden_colName' value=''></td></tr>");
               
               //버튼을 추가
               $(this).parent().parent().parent().parent().append("<tr><td colspan='3'><center><input id='sub"+ tableNumber +"' type='button' class='btn btn-default' value='전송' style='width: 100px;'> <input type='button' class='btn btn-default' id='plus"+ tableNumber +"' value='추가' style='width: 100px; '><input type='hidden' value='"+colNumber+"' id='ta_col_"+tableNumber+"'></center></td></tr>");
               
               //다이얼 로그 추가
               $(this).parent().parent().parent().parent().parent().parent().parent().append("<div hidden id='constraintDialog"+ tableNumber +"_"+ colNumber +"' title='제약조건'>PRIMARY KEY <input type='radio' name='constraint_ck' id='constraint_ck' value='PRIMARY KEY'>   FOREIGN KEY <input type='radio' name='constraint_ck' id='constraint_ck' value='FOREIGN KEY'>   NOT NULL <input type='radio' name='constraint_ck' id='constraint_ck' value='NOT NULL'></div>");
